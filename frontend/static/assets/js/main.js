...
if (window.location.pathname.includes('edit-course.html')) {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        // This is now the main entry point for the page's script
        ...
        // --- THIS IS THE MISSING LOGIC FOR THE UPLOAD BUTTON ---
        const uploadExerciseBtn = document.getElementById('upload-exercise-btn');
        const lessonExerciseFileInput = document.getElementById('lesson-exercise-file');
        const exerciseFileNameDisplay = document.getElementById('exercise-file-name');
        ...
        // (This section includes all your other existing logic for saving, deleting, etc.)
        
        // Example: Lesson Modal Setup
        if (lessonModal) {
            lessonModal.addEventListener('show.bs.modal', (e) => {
                const button = e.relatedTarget;
                if (button && button.classList.contains('add-lesson-btn')) {
                    currentEditingEpisodeId = button.dataset.episodeId;
                    // ... rest of your modal setup logic ...
                }
            });
        }
        
        // ... ALL of your other functions and event listeners for this page go here ...
        // (renderCourseBuilder, saveLessonBtn listener, deleteTopic listener, etc.)
        ...
        fetch(`${API_BASE_URL}/api/courses/edit/${courseId}`, { headers: { 'x-auth-token': token } })
            ...
            .then(result => {
                if (result.success) {
                    ...
                    courseData = course; // Store course data globally for this page

                    // ... all of your code to populate the form fields ...
                    document.getElementById('field-1').value = course.title || '';
                    document.getElementById('aboutCourse').value = course.description || '';
                    // etc.
