...
if (window.location.pathname.includes('edit-course.html')) {
    ...
    // --- Helper function to render the course builder UI ---
    const renderCourseBuilder = (episodes) => {
        ...
        container.innerHTML = episodes.map((episode) => {
            const lessonsHtml = episode.lessons.map(lesson => {
                ...
                return `<div class="d-flex justify-content-between rbt-course-wrape mb-4">
                            ...
                            <div class="col-2 inner">
                                <ul class="rbt-list-style-1 rbt-course-list d-flex gap-2 justify-content-end">
                                    <li><i class="feather-trash delete-lesson" data-episode-id="${episode._id}" data-lesson-id="${lesson._id}" style="cursor: pointer;"></i></li>
                                    <li><i class="feather-edit edit-lesson" data-episode-id="${episode._id}" data-lesson-id="${lesson._id}" style="cursor: pointer;"></i></li>
                                </ul>
                            </div>
                        </div>`;
            }).join('');
            return `<div class="accordion-item card mb--20">
                        <h2 class="accordion-header card-header rbt-course">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#episode-collapse-${episode._id}">${episode.title}</button>
                        ...
                        <div id="episode-collapse-${episode._id}" class="accordion-collapse collapse">
                            <div class="accordion-body card-body">
                                ...
                                <button class="rbt-btn btn-border hover-icon-reverse rbt-sm-btn-2 add-lesson-btn" type="button" data-bs-toggle="modal" data-bs-target="#Lesson" data-episode-id="${episode._id}">
                                    <span class="icon-reverse-wrapper"><span class="btn-text">Add Lesson</span><span class="btn-icon"><i class="feather-plus-square"></i></span><span class="btn-icon"><i class="feather-plus-square"></i></span></span>
                                </button>
                            </div>
                        </div>
                    </div>`;
    ...
    // --- Helper function to open the 'Edit Lesson' modal ---
    ...
    window.onload = function() {
        ...
        if (!token || user.role !== 'instructor' || !courseId) {
            alert("Access Denied or Course ID missing.");
            window.location.href = 'login.html';
            return;
        }

        // --- FETCH AND POPULATE PAGE ON LOAD ---
        ...
        // Custom "Upload Exercise File" button
        const uploadExerciseBtn = document.getElementById('upload-exercise-btn');
        const lessonExerciseFileInput = document.getElementById('lesson-exercise-file');
        const exerciseFileNameDisplay = document.getElementById('exercise-file-name');
        if (uploadExerciseBtn && lessonExerciseFileInput) {
            uploadExerciseBtn.addEventListener('click', () => lessonExerciseFileInput.click());
            lessonExerciseFileInput.addEventListener('change', () => {
                exerciseFileNameDisplay.textContent = lessonExerciseFileInput.files.length > 0 ? `Selected: ${lessonExerciseFileInput.files[0].name}` : '';
            });
        }
        
        // "Remove Exercise File" button
        ...
        document.getElementById('save-lesson-btn')?.addEventListener('click', async () => {
            ...
            formData.append('summary', document.getElementById('lesson-summary').value);
            formData.append('vimeoUrl', document.getElementById('lesson-video-url').value);
            // ... append other lesson fields ...
            formData.append('removeExerciseFile', document.getElementById('remove-exercise-file-flag').value);
            if (lessonExerciseFileInput.files[0]) {
                formData.append('exerciseFile', lessonExerciseFileInput.files[0]);
            }
            // ... logic to POST or PUT the form data ...
        });

        // ... Add your other event listeners here (save topic, delete lesson, etc.)
    };
}
