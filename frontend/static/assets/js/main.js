        ...
        fetch(`${API_BASE_URL}/api/courses/edit/${courseId}`, { headers: { 'x-auth-token': token } })
            ...
            .then(result => {
                if (result.success) {
                    ...
                    // --- ADD THESE NEW LINES FOR THE LOGO ---
const logoPreview = document.getElementById('courseLogoPreview');
if (logoPreview && course.courseLogo) {
    logoPreview.src = `/${course.courseLogo}`;
}
// --- END OF NEW LINES ---
        ...
        if (editCourseForm) {
            editCourseForm.addEventListener('submit', async (e) => {
                ...
                // --- ADD THESE NEW LINES FOR THE LOGO ---
const courseLogoInput = document.getElementById('courseLogoInput');
if (courseLogoInput.files.length > 0) {
    formData.append('courseLogo', courseLogoInput.files[0]);
}
// --- END OF NEW LINES ---
...
if (window.location.pathname.includes('lesson.html')) {
    ...
    function renderSidebar(activeLessonId, activeQuizId) {
        ...
        if (!sidebar) return;
        const activeEpisode = currentCourseData.episodes.find(ep => ep.lessons.some(l => l._id === activeLessonId) || ep.quizzes.some(q => q._id === activeQuizId));
        sidebar.innerHTML = currentCourseData.episodes.map((episode, index) => {
            ...
            const contentHTML = contents.map(content => {
                ...
                return `<li><a href="#" class="content-link ${isActive ? 'active' : ''}" data-type="${content.type}" data-id="${content._id}"><div class="course-content-left"><i class="feather-${icon}"></i><span class="text">${content.title}</span></div><div class="course-content-right"><span class="rbt-check unread"><i class="feather-circle"></i></span></div></a></li>`;
            }).join('');
            return `<div class="accordion-item card"><h2 class="accordion-header card-header"><button class="accordion-button ${isExpanded ? '' : 'collapsed'}" type="button" data-bs-toggle="collapse" data-bs-target="#collapseSidebar${index}">${episode.title}</button></h2><div id="collapseSidebar${index}" class="accordion-collapse collapse ${isExpanded ? 'show' : ''}"><div class="accordion-body card-body"><ul class="rbt-course-main-content liststyle">${contentHTML}</ul></div></div></div>`;
        }).join('');
    }
   ...
   function updateLessonContent(lessonId) {
    ...
    if (selectedLesson.vimeoUrl) {
        ...
        // Add parameters to clean up Vimeo player (hide title/byline to remove any potential top bar space)
        const embedUrl = `https://player.vimeo.com/video/${videoId}?title=0&byline=0&portrait=0&autopause=0`;
        const videoHTML = `<div class="plyr__video-embed rbtplayer"><iframe src="${embedUrl}" allowfullscreen allow="autoplay; fullscreen; picture-in-picture"></iframe></div>`;
        let resourcesHTML = '';
        if (selectedLesson.exerciseFiles && selectedLesson.exerciseFiles.length > 0) {
            resourcesHTML = `<div class="rbt-lesson-attachments mt--30"><h5 class="rbt-title-style-3">Lesson Resources</h5><ul class="rbt-list-style-1">${selectedLesson.exerciseFiles.map(file => `<li><a href="#" class="lesson-pdf-link" data-file-path="/${file.path}" data-file-name="${file.filename}"><i class="feather-paperclip"></i> ${file.filename}</a></li>`).join('')}</ul></div>`;
        }
        const descriptionHTML = `<div class="content"><div class="section-title"><h4>About Lesson</h4><p>${selectedLesson.summary || 'No summary available.'}</p></div>${resourcesHTML}</div>`;
        contentContainer.innerHTML = videoHTML + descriptionHTML;

        // Initialize Plyr after HTML insertion
        const player = new Plyr('.rbtplayer', {
            controls: ['play-large', 'play', 'progress', 'current-time', 'mute', 'volume', 'fullscreen'],
            vimeo: { preloop: false, autoplay: false }, // Vimeo-specific options
            ratio: '16:9', // Enforce aspect ratio
            clickToPlay: true,
            muted: false,
            volume: 1
        });

        // Optional: Adjust styles after Plyr is ready (if any post-load gap persists)
        player.on('ready', () => {
            // Force no top margin/padding on video container
            const videoContainer = document.querySelector('.rbtplayer');
            if (videoContainer) {
                videoContainer.style.marginTop = '0';
                videoContainer.style.paddingTop = '0';
            }
            // Ensure description is directly below with no gap
            const contentElem = document.querySelector('.rbt-lesson-rightsidebar .content');
            if (contentElem) {
                contentElem.style.marginTop = '0';
            }
        });
    ...
    function renderQuizStartScreen(quizId) {
        ...
        // **FIX**: Remove the 'lesson-video' class so quizzes have correct padding.
        rightSidebar.classList.remove('lesson-video');
...
const renderCourseCurriculum = (episodes) => {
    ...
    container.innerHTML = episodes.map((episode, index) => {
        const lessonsHtml = episode.lessons.map(lesson => `
            <li>
                <a href="#" data-lesson-id="${lesson._id}">
                    <div class="course-content-left">
                        <i class="feather-play-circle"></i> <span class="text">${lesson.title}</span>
                    </div>
                    <div class="course-content-right">
...
const populateInstructor = (instructor) => {
    ...
    if (socialContainer && instructor.social) {
        ...
        .then(result => {
            ...
            // Add this after renderCourseCurriculum(...)
document.querySelector('#coursecontent').addEventListener('click', (e) => {
