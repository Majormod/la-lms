...
if (window.location.pathname.includes('edit-course.html')) {
    ...
    window.onload = function() {
        ...
        let courseData = null; // This will hold the full course object

        if (!courseId || !token) {
            alert("Access Denied or Course ID missing.");
            window.location.href = '/login.html';
            return;
        }
        ...
        const renderCourseBuilder = (episodes) => {
            ...
            container.innerHTML = episodes.map((episode) => {
                const lessonsHtml = episode.lessons.map(lesson => `<div class="d-flex justify-content-between rbt-course-wrape mb-4"><div class="col-10 inner d-flex align-items-center gap-2"><i class="feather-play-circle"></i><h6 class="rbt-title mb-0">${lesson.title}</h6></div><div class="col-2 inner"><ul class="rbt-list-style-1 rbt-course-list d-flex gap-2"><li><i class="feather-trash"></i></li><li><i class="feather-edit"></i></li></ul></div></div>`).join('');
                return `<div class="accordion-item card mb--20"><h2 class="accordion-header card-header rbt-course"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#episode-collapse-${episode._id}">${episode.title}</button><span class="rbt-course-icon rbt-course-edit" data-bs-toggle="modal" data-bs-target="#UpdateTopic" data-episode-id="${episode._id}"></span><span class="rbt-course-icon rbt-course-del" data-episode-id="${episode._id}"></span></h2><div id="episode-collapse-${episode._id}" class="accordion-collapse collapse"><div class="accordion-body card-body">${lessonsHtml || '<p class="mb-4">No lessons.</p>'}<div class="d-flex"><button class="rbt-btn btn-border hover-icon-reverse rbt-sm-btn-2 add-lesson-btn" type="button" data-bs-toggle="modal" data-bs-target="#Lesson" data-episode-id="${episode._id}"><span class="icon-reverse-wrapper"><span class="btn-text">Add Lesson</span><span class="btn-icon"><i class="feather-plus-square"></i></span><span class="btn-icon"><i class="feather-plus-square"></i></span></span></button></div></div></div></div>`;
            }).join('');
        };
        ...
        fetch(`${API_BASE_URL}/api/courses/edit/${courseId}`, { headers: { 'x-auth-token': token } })
            ...
            .then(result => {
                if (result.success) {
                    ...
                    document.getElementById('field-1').value = courseData.title || '';
                    document.getElementById('field-2').value = courseData.slug || '';
                    document.getElementById('aboutCourse').value = courseData.description || '';
                    document.getElementById('regularPrice-1').value = courseData.originalPrice || '';
                    document.getElementById('discountedPrice-1').value = courseData.price || '';
                    document.getElementById('videoUrl').value = courseData.previewVideoUrl || '';
                    document.getElementById('course-status').value = courseData.status || 'Draft';
                    if (courseData.thumbnail) document.getElementById('createfileImage').src = `/${courseData.thumbnail}`;
                    renderCourseBuilder(courseData.episodes);
                } else { throw new Error(result.message); }
            })
            .catch(error => {
                console.error('Error loading course data:', error);
                alert('Error loading course data. Redirecting...');
                window.location.href = 'instructor-course.html';
            });
        ...
        if (saveTopicBtn) {
            saveTopicBtn.addEventListener('click', async () => {
                ...
                try {
                    const response = await fetch(`${API_BASE_URL}/api/courses/${courseId}/episodes`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-auth-token': token }, body: JSON.stringify({ title, summary }) });
                    const result = await response.json();
                    if (result.success) {
                        courseData = result.course;
                        renderCourseBuilder(courseData.episodes);
                        bootstrap.Modal.getInstance(addTopicModal).hide();
                        addTopicModal.querySelector('form')?.reset();
                    } else { alert(`Error: ${result.message}`); }
                } catch (error) { alert('An error occurred while saving the topic.'); }
            });
        }
        
        // --- UNIFIED EVENT LISTENER FOR DYNAMIC BUTTONS (EDIT & DELETE) ---
        document.addEventListener('click', async (e) => {
            // Logic for Topic Delete Button
            ...
            if (deleteBtn) {
                if (confirm('Are you sure you want to delete this topic?')) {
                    ...
                    } catch (error) { alert('An error occurred while deleting the topic.'); }
                }
            }
            // Logic for Topic Edit Button (pre-fills the modal)
            const editBtn = e.target.closest('.rbt-course-edit');
            if (editBtn && courseData) {
                const episodeId = editBtn.dataset.episodeId;
                ...
                if (topic) {
                    ...
                    saveUpdateTopicBtn.dataset.episodeId = episodeId;
                }
            }
        });
        
        // --- "UPDATE TOPIC" MODAL LOGIC ---
        if(saveUpdateTopicBtn) {
            saveUpdateTopicBtn.addEventListener('click', async () => {
                const episodeId = saveUpdateTopicBtn.dataset.episodeId;
                ...
                } catch (error) { alert('An error occurred while updating the topic.'); }
            });
        }
        ...
        document.getElementById('create-course-form')?.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/api/courses/${courseId}`, { method: 'PUT', headers: { 'x-auth-token': token }, body: formData });
                const result = await response.json();
                if (result.success) {
                    alert('Course updated successfully!');
                    window.location.href = '/instructor-course.html';
                } else { alert(`Error: ${result.message}`); }
            } catch (error) { alert('An error occurred while updating the course.'); }
        });
    };
}
