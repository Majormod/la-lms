// This is the entire, final content for ecommerce.js
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // All cart logic remains the same... (no changes needed here)
    const Cart = {
    ...
    // Global click listeners... (no changes needed here)
    ...
    // FINAL VERSION: LOGIC FOR COURSE DETAILS (Lock Icon)
    if (path.includes('course-details.html') || path.includes('the-masterclass-details.html')) {
        const urlParams = new URLSearchParams(window.location.search);
        const courseId = urlParams.get('courseId');
        if (courseId) {
            fetch(`${API_BASE_URL}/api/courses/${courseId}`, { headers: { 'x-auth-token': token } })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                        ...
                        course.episodes.forEach((episode, index) => {
                            ...
                            const contentHtml = allContents.map(content => {
                                ...
                                if (hasAccess || content.isPreview) {
                                    ...
                                    return `<li class="locked"><a href="#" class="disabled"><div class="course-content-left"><i class="${iconClass}"></i> <span class="text">${content.title}</span></div><div class="course-content-right">${lockIcon}</div></a></li>`;
                                }
                            }).join('');
