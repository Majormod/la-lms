		...
		updateUI: function() {
			...
			if (miniCartWrapper && miniCartFooter) {
				if (cart.length === 0) {
					...
					cart.forEach(item => {
						const itemHtml = `<li class="minicart-item"><div class="thumbnail"><a href="${item.url}"><img src="/${item.thumbnail}" alt="${item.title}"></a></div><div class="product-content"><h6 class="title"><a href="${item.url}">${item.title}</a></h6><span class="quantity">${item.quantity} * <span class="price">₹${item.price.toLocaleString('en-IN')}</span></span></div><div class="close-btn"><button class="rbt-round-btn remove-from-cart-btn" data-item-id="${item.id}"><i class="feather-x"></i></button></div></li>`;
				...
				if (data.success) {
					...
					if (curriculumContainer) {
						...
						course.episodes.forEach((episode, index) => {
							const allContents = [...(episode.lessons || []).map(item => ({
								...item,
								type: 'lesson'
							})), ...(episode.quizzes || []).map(item => ({
								...item,
								type: 'quiz'
							}))];
...
const createCourseCardHTML = (course) => {
    // This line is correct, it checks the status
            ...
            <div class="rbt-card-body">
                ...
                <div class="rbt-card-bottom">
                    ${isCompleted ? 
                        ...
                        `<a class="rbt-btn btn-sm btn-border-gradient w-100 text-center" 
                            href="course-details.html?courseId=${course._id}">
                            Continue Course
                        </a>`
                    }
                </div>
		...
		if (cartTableBody) {
			...
			if (cart.length === 0) {
				...
				cart.forEach(item => {
					...
					const row = `<tr><td class="pro-title"><a href="${item.url}" style="display: flex; align-items: center;"><img src="/${item.thumbnail}" alt="${item.title}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 5px; margin-right: 15px;"> ${item.title}</a></td><td class="pro-price"><span>₹${item.price.toLocaleString('en-IN')}</span></td><td class="pro-quantity"><div class="pro-qty"><span class="dec qtybtn" data-item-id="${item.id}">-</span><input type="number" value="${item.quantity}" data-item-id="${item.id}" class="quantity-input" readonly><span class="inc qtybtn" data-item-id="${item.id}">+</span></div></td><td class="pro-subtotal"><span>₹${itemTotal.toLocaleString('en-IN')}</span></td><td class="pro-remove"><a href="#" class="remove-from-cart-btn" data-item-id="${item.id}"><i class="feather-x"></i></a></td></tr>`;
