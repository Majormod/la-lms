...
console.log('JWT Secret loaded by server:', process.env.JWT_SECRET);
...
// 5. Uploader for Course Logos
const logoUpload = multer({
    storage: multer.diskStorage({
        destination: (req, file, cb) => {
            const uploadPath = path.join(__dirname, '../frontend/static/uploads/logos');
            fs.mkdirSync(uploadPath, { recursive: true });
            cb(null, uploadPath);
        },
        filename: (req, file, cb) => {
            cb(null, `${Date.now()}-logo-${file.originalname.replace(/\s+/g, '_')}`);
        }
    }),
});
...
app.get('/api/courses', async (req, res) => {
    try {
        ...
        // --- [ADDED] School Filter ---
        // NOTE: This assumes you have a field named 'school' in your Course model in MongoDB.
        if (req.query.school) {
        ...
        // If there are any price conditions, add them to the main filters using $and
        ...
        switch (req.query.sortBy) {
            ...
            case 'price_desc':
                sortOptions = { price: -1 };
                break;
            // Add other sort options like 'latest' if needed
            case 'latest':
            default:
        ...
        storage: multer.diskStorage({
            destination: function (req, file, cb) {
                ...
                if (file.fieldname === 'thumbnail') {
                    dest = path.join(__dirname, '../frontend/static/uploads/thumbnails');
                } else if (file.fieldname === 'courseLogo') {
                    dest = path.join(__dirname, '../frontend/static/uploads/logos');
                }
    ...
    async (req, res) => {
        try {
            ...
            if (req.files) {
                ...
                if (req.files.courseLogo && req.files.courseLogo[0]) {
                    course.courseLogo = `uploads/logos/${req.files.courseLogo[0].filename}`;
                }
            }
